...
namespace Microsoft.Dafny {
  public abstract class Compiler {
    ...
    public bool IsTlaCompiler() {
      return String.Equals(TargetLanguage, "TLA");
    }
    ...
    public void Compile(Program program, ConcreteSyntaxTree wrx) {
      ...
      foreach (ModuleDefinition m in program.CompileModules) {
        Console.WriteLine(String.Format("    TONY: Compiling module `{0}` as `{1}`", m.DafnyName, m.Name));
        // Console.WriteLine();
        // Console.WriteLine(Printer.ModuleDefinitionToString(m));
        // Console.WriteLine();

        if (IsTlaCompiler() && String.Equals(m.Name, "_System")) {
          // TLA does not need Dafny System module
          Console.WriteLine("    TONY: TLA does not need Dafny System module. Skip");
          continue;
        }
        if (m.IsAbstract) {
          // the purpose of an abstract module is to skip compilation
          Console.WriteLine(String.Format("        TONY: Module module {0} is abstract. Skipping", m.DafnyName));
          continue;
        }
        if (!m.IsToBeCompiled) {
          Console.WriteLine(String.Format("        TONY: Module module {0} is !IsToBeCompiled. Skipping", m.DafnyName));
          continue;
        }
        ...
        foreach (TopLevelDecl d in m.TopLevelDecls) {
          ...
          if (d is OpaqueTypeDecl) {
            Console.WriteLine("        TONY: OpaqueTypeDecl");
            ...
            Console.WriteLine("        TONY: ValuetypeDecl (ignore)");
            // nop
            continue;
          } else if (d is ModuleDecl) {
            Console.WriteLine("        TONY: ModuleDecl (ignore)");
            // nop
            continue;
    ...
    void CompileClassMembers(Program program, TopLevelDeclWithMembers c, IClassWriter classWriter) {
      ...
      Console.WriteLine("            TONY: CompileClassMembers for {0}", c.Name);
      ...
      foreach (MemberDecl memberx in c.Members) {
        ...
        if (c is TraitDecl && member.OverriddenMember != null && !member.IsOverrideThatAddsBody) {
          ...
          if (f.Body == null && !(c is TraitDecl && !f.IsStatic) &&
            ...
            // nothing to compile (unless TLA), but we do check for assumes
            if (f.Body == null) {
